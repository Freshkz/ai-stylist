...
if __name__ == "__main__":
    try:
        ...
        log_path = os.path.join(BASE_DIR, "admin_startup_error.log")
        with open(log_path, "w", encoding="utf-8") as fh:
            fh.write(f"Error al abrir la app: {exc}\n")
        try:
            import traceback
            traceback.print_exc()
            root = tk.Tk()
            root.withdraw()
            messagebox.showerror("AI Stylist Admin", f"La app falló al abrirse.\n\nDetalles guardados en: {log_path}\n\n{exc}")
            root.destroy()
        except Exception:
            pass
        raise
